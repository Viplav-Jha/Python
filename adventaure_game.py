name = input("Type your name:  ")
print("Welcome", name , "to this adventure !")

answer = input("You are  on a dirt road , it come to end and you can go left or right.Which way would  you like  to go?  ").lower()

if answer == 'left':
    answer = input("You come to river , you can walk around it or swim  accross ? Type walk to walk around  and swim to swim accross :")
    
    if answer == 'swim':
        print("You swim accross and were eaten by an alligator.")
    elif answer == "walk":
        print("You walked for many miles , ran out of water  and you lost the game. ")
    else:
        print('Not a vaild option. You lose')
elif answer == 'right':
    answer = input("You come to a bridege , it looks wobbly , do you  want to cross it or or head back (cross/back)? ")

    if answer == 'back':
            print("You go back and lose.")
    elif answer == "cross":
         answer = input("You cross a bridge and meet a stranger. Do you talk them(yes/no) " )
         if answer == "yes":
                print("You talk to stranger and they give you gold. You WIN !")
         elif answer =="no":
                print("You ignore the stranger and they offended and You lose.")
         else:
            print('Not a vaild option. You lose')
    else:
        print('Not a vaild option. You lose')

else:
    print('Not a vaild option. You lose')